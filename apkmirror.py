...
class APKMirror:
    ...
    def get_app_details(self, app_link):
        ...
        data = soup.find_all("div", {"class": ["table-row", "headerFont"]})[2] #change to APK without BUNDLE, default is 1
    ...
    def download_file_by_directlink(self, directlink_url, filename):
        print("[download_file_by_direct_download_link] Sleeping...")

        time.sleep(self.timeout)

        resp = self.scraper.get(directlink_url, headers=self.headers)
        cfurl = resp.content
